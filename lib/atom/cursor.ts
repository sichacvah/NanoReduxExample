import { assocPath, path as get } from 'ramda'
import type {Atom, Listener, IDisposable} from './types'

export const cursor = <S extends object, T>(
  source: Atom<S>,
  path: (string | number)[],
): Atom<T> & IDisposable => {
  const extract = (obj: S): T => get(path, obj) as T
  ...
  const thisAtom: Atom<T> & IDisposable = {
    ...
    swap: (fn, ...args) => {
      source.swap((state, ...rest) => {
        const next = assocPath(path, fn(extract(state), ...rest), state)
        return next
      }, ...args)
      return thisAtom
    },
    reset: (next: T) => {
      ...
      source.reset(assocPath(path, next, current))
      return thisAtom
    },
  }
  return thisAtom
